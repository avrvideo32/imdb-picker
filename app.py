...
if generate_btn:
    ...
    st.info("👈 Adjust the filters in the sidebar and click **Generate Picks** to find your next watch!")import streamlit as st
import duckdb
import html
from filters import build_where
from utils import (
...
# --- SIDEBAR (Filters & Form) ---
with st.sidebar:
    # Using st.form batches inputs and prevents the app from rerunning on every click
    with st.form("filter_form"):
        st.header("🎛️ Curation Filters")
        ...
        selected_types = st.multiselect("Types", TITLE_TYPES, default=DEFAULT_TYPES)
        selected_genres = st.multiselect("✅ Include Genres", GENRES)
        excluded_genres = st.multiselect("🚫 Exclude Genres", GENRES)
        
        # Moved sort and picks into the form so changing them doesn't clear results
        SORT_OPTIONS = {
...
if generate_btn:
    ...
    where, params = build_where(values, selected_genres, selected_types)
    order_clause = SORT_OPTIONS[sort_by]
    ...
    with st.spinner("Querying database..."):
        ...
        if total == 0:
            ...
            # --- RESULTS UI ---
            for _, row in df.iterrows():
                # html.escape prevents XSS vulnerabilities from malicious titles
                title = html.escape(str(row['primaryTitle']))
                yr = html.escape(str(row['startYear']))
                genres = html.escape(str(row['genres']).replace(',', ' • '))
                rt = format_runtime(row['runtimeMinutes'], as_hms=True) or 'N/A'
    ...
    # Welcome state before the user generates their first pick
